...
def recommend_books(book_title):
    ...
    cos_sim = cosine_similarity(pivot_data.iloc[:, :book_title_index].join(pivot_data.iloc[:, book_title_index+1:]), 
                                pivot_data.iloc[:, book_title_index].values.reshape(1, -1))
    svd_similar_books_index = np.unique(np.argsort(cos_sim)[:, -6:-1].reshape(-1))
