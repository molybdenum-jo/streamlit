...
# 도서와 사용자 각각에 대한 ID를 매핑하는 딕셔너리 생성
book_to_idx = {}
user_to_idx = {}

unique_books = train['Book-Title'].unique()
unique_users = train['User-ID'].unique()
...
for row in train.itertuples():
    book_idx = book_to_idx[row[2]]
    user_idx = user_to_idx[row[1]]
    ratings[book_idx, user_idx] = row[3]
...
# 모델 학습
history = model.fit(ratings, ratings, epochs=10, batch_size=128)

# 임베딩 벡터 추출
embedding_model = Model(inputs, x)
book_embeddings = embedding_model.predict(ratings)

# 추천 함수 정의
def recommend_books(user_id, n=5):
    user_idx = user_to_idx[user_id]
    user_embedding = book_embeddings[:, user_idx]
    scores = np.dot(book_embeddings.T, user_embedding)
    top_n = np.argsort(scores)[-n:]
    top_n_books = unique_books[top_n]
    return top_n_books
